...
pct_p = lambda v: f"{float(v):.1f}%" if v not in (None,'') else "n/d"  # já em %
...
# ══════════════════════════════════════════════════════════════════
# COBERTURA — Visão Geral + Empresa + Fundamentals ComDinheiro
# ══════════════════════════════════════════════════════════════════
if pagina == "Cobertura":
    ...
    with _ctab1:
            ...
            for emp in empresas_vg:
                ...
                rows.append({
                    ...
                    "ROIC": pct(r.get("roic")), "Mg EBIT": pct_p(r.get("ebit_margin")),
                    "WACC": pct_p(r.get("wacc")), "Beta": f_(r.get("beta")), "Rec.": _rec,
                })
                rows_html.append({
                    ...
                    "Mg EBIT":   pct_p(r.get("ebit_margin")), "WACC": pct_p(r.get("wacc")),
                    "Beta":      f_(r.get("beta")),
    ...
    with _ctab2:
            ...
            emp_sel = st.selectbox("Cobertura", empresas,
                                   index=_default_idx,
            ...
            c5.metric("WACC",         pct_p(r.get("wacc")))
            c6.metric("Beta",         f_(r.get("beta")))
            c1,c2,c3,c4,c5,c6=st.columns(6)
            c1.metric("Margem EBIT",  pct_p(r.get("ebit_margin")))
            c2.metric("Margem Líq.",  pct(r.get("net_margin")))
            c3.metric("ROIC",         pct(r.get("roic")))
            c4.metric("ROE",          pct_p(r.get("roe")))
...
# ══════════════════════════════════════════════════════════════════
                ...
                if emps_cmp:
                    ...
                    for ec in emps_cmp:
                        ...
                        rows_cmp.append({
                            ...
                            "Upside": f"{float(rc.get('upside') or 0):+.1f}%",
                            "WACC": pct_p(rc.get("wacc")),
                            "Mg EBIT": pct_p(rc.get("ebit_margin")),
    ...
    with _atab2:
            ...
            for emp in empresas:
                r=results[emp]; ticker=r.get("ticker",emp)
                up=float(r.get("upside") or 0); wac=float(r.get("wacc") or 0)
...
# ══════════════════════════════════════════════════════════════════
    ...
    _mtab1, _mtab2, _mtab3, _mtab4 = st.tabs([
        "Exposição Geográfica", "Governança", "Gestoras", "Acionistas"
    ])
    with _mtab1:
    ...
    with _mtab3:
            ...
            emp_g = st.selectbox("Cobertura", empresas,
    ...
    with _mtab4:
        st.markdown("### Composição Acionária")
        _emp_ac = st.selectbox("Cobertura", list(results.keys()), key="ac_emp_sel")
        if st.button("Carregar Acionistas", type="primary", key="ac_load_btn"):
            _tk_ac = results[_emp_ac].get("ticker","").replace(".SA","")
            if _tk_ac:
                with st.spinner("Buscando via ComDinheiro..."):
                    import asyncio as _aio, sys as _sys
                    _sys.path.insert(0,"/opt/shipyard")
                    import comdinheiro_scraper as _cds
                    _ac_data = _aio.run(_cds.get_acionistas(_tk_ac))
                if _ac_data.get("acionistas"):
                    import pandas as _pdac
                    _df_ac = _pdac.DataFrame(_ac_data["acionistas"])
                    st.caption(_ac_data.get("data_posicao",""))
                    _cols_ac = [c for c in ["rank","nome","pct_on","pct_pn","pct_total","controle","pais"] if c in _df_ac.columns]
                    st.markdown(dark_table(_df_ac[_cols_ac]), unsafe_allow_html=True)
                    export_buttons(_df_ac, "acionistas_" + _tk_ac)
                else:
                    st.warning(f"Sem dados de acionistas para {_tk_ac}.")
            else:
